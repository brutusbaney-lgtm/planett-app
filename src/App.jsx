...
const firebaseConfig = {
  ...
  authDomain: "planett-football.firebaseapp.com",
  projectId: "planett-football",
  storageBucket: "planett-football.firebasestorage.app",
  messagingSenderId: "883055023158",
  appId: "1:883055023158:web:47514131114b44e5dd7497",
};
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"60px 28px 48px", background:"radial-gradient(ellipse at 50% 0%, "+C.accentGlow+" 0%, transparent 55%), "+C.bg }}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:22 }}>
        ...
        <div style={{ width:"100%", display:"flex", gap:8, background:C.surface, borderRadius:12, padding:4 }}>
          {["signin","signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(""); }} style={{ flex:1, padding:"10px", borderRadius:9, border:"none", background:mode===m?C.accent:"transparent", color:mode===m?C.bg:C.muted, fontSize:14, fontWeight:700, cursor:"pointer" }}>
              {m==="signin"?"Sign In":"Create Account"}
            </button>
          ))}
        </div>
        <div style={{ width:"100%", display:"flex", flexDirection:"column", gap:12 }}>
          <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email address" type="email" style={{ width:"100%", padding:"16px", borderRadius:12, background:C.surface, border:"1.5px solid "+C.border, color:C.text, fontSize:15, outline:"none", boxSizing:"border-box" }}/>
          <input value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password" type="password" style={{ width:"100%", padding:"16px", borderRadius:12, background:C.surface, border:"1.5px solid "+C.border, color:C.text, fontSize:15, outline:"none", boxSizing:"border-box" }}/>
          {error && <div style={{ color:C.red, fontSize:13, textAlign:"center" }}>{error}</div>}
          <button onClick={handleSubmit} disabled={loading||!email||!password} style={{ width:"100%", padding:"18px", borderRadius:16, background:C.accent, border:"none", color:C.bg, fontSize:16, fontWeight:800, letterSpacing:1.5, cursor:"pointer", boxShadow:"0 4px 24px "+C.accentGlow, opacity:loading||!email||!password?0.6:1 }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const pd = await getDoc(doc(db, "users", user.uid, "data", "player"));
        if (pd.exists()) setPlayerState(pd.data());
      } catch(e){}
      try {
        const fd = await getDocs(collection(db, "users", user.uid, "fixtures"));
        setFixturesState(fd.docs.map(d => ({ id: d.id, ...d.data() })));
      } catch(e){}
      try {
        const hd = await getDocs(collection(db, "users", user.uid, "gameHistory"));
        const sorted = hd.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => (b.date||"").localeCompare(a.date||""));
        setGameHistoryState(sorted);
      } catch(e){}
    };
    load();
  }, [user]);
  ...
  const savePlayer = async (p) => {
    if (!user) return;
    try { await setDoc(doc(db, "users", user.uid, "data", "player"), p); } catch(e){}
  };
  ...
  const handleGameSaved = async (gameRecord, fixtureId) => {
    const newGame = { ...gameRecord, fixtureId };
    setGameHistoryState(prev => [newGame, ...prev]);
    if (user) {
      try { await addDoc(collection(db, "users", user.uid, "gameHistory"), newGame); } catch(e){}
    }
    if (fixtureId) {
      setFixturesState(prev => prev.map(f => f.id === fixtureId ? { ...f, status:"completed" } : f));
      if (user) {
        try { await updateDoc(doc(db, "users", user.uid, "fixtures", fixtureId), { status:"completed" }); } catch(e){}
      }
    }
  };

  const handleFixtureSaved = async (fixture) => {
    if (user) {
      try {
        const ref = await addDoc(collection(db, "users", user.uid, "fixtures"), fixture);
        setFixturesState(prev => [...prev, { ...fixture, id: ref.id }]);
      } catch(e){ setFixturesState(prev => [...prev, fixture]); }
    } else {
      setFixturesState(prev => [...prev, fixture]);
    }
  };
  ...
  if (phase==="splash") return shell(<SplashScreen onDone={() => { if (user) setPhase("app"); else setPhase("login"); }}/>, false);
  if (phase==="login" || !user)  return shell(<LoginScreen onLogin={() => { setPhase("app"); }}/>, false);
